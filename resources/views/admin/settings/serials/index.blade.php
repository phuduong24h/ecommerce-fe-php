        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden">
            <table class="w-full">
                <thead class="border-b bg-muted/50">
                    <tr>
                        <th
                            class="h-12 px-4 text-left align-middle font-medium text-muted-foreground text-xs uppercase tracking-wider">
                            Số serial</th>
                        <th
                            class="h-12 px-4 text-left align-middle font-medium text-muted-foreground text-xs uppercase tracking-wider">
                            Sản phẩm</th>
                ...
                <tbody id="serialsTable" class="divide-y">
                    @forelse($serials as $serial)
                        @php
                            $statusMapping = [
                                'available' => ['text' => 'Còn hàng', 'class' => 'bg-emerald-100 text-emerald-700'],
                                'sold' => ['text' => 'Đã bán', 'class' => 'bg-blue-100 text-blue-700'],
                                'warranty_claimed' => ['text' => 'Đã bảo hành', 'class' => 'bg-amber-100 text-amber-700'],
                            ];

                            $statusKey = strtolower($serial['status']); // <-- chuyển status API về lowercase
                            $statusClass = $statusMapping[$statusKey]['class'] ?? 'bg-gray-100 text-gray-700';
                            $statusText = $statusMapping[$statusKey]['text'] ?? 'Chưa xác định';
                        @endphp
                        ...
                        <tr class="hover:bg-muted/50 transition-colors">
                            ...
                            <td class="p-4 align-middle">
                                <div class="flex gap-2">
                                    ...
                                    <form action="{{ route('admin.settings.serials.destroy', $serial['id']) }}"
                                        method="POST"
                                        onsubmit="return confirm('Bạn có chắc chắn muốn xóa serial này không?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
